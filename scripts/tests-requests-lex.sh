...
cargo build -q --example lex_request

BIN="${BIN:-./target/debug/examples/lex_request}"
DIFF="diff -u"
...
for file_in in ./requests/**/*.http; do
    ...
    expected_out="./requests/$base.http.lex.out"
    expected_err="./requests/$base.http.lex.err"
    expected_exit="./requests/$base.http.lex.exit"
    argv_file="./requests/$base.http.argv"
    env_file="./requests/$base.http.env"
    input_file="./requests/$base.http.in"

    actual_out="$TMP_DIR/$base.http.lex.out"
    mkdir -p "$(dirname "$actual_out")"

    actual_err="$TMP_DIR/requests/$base.http.lex.err"
